...
LiquidCrystal_I2C lcd(0x27, 20, 4);  // 0x27 is the I2C address for the display
...
void setup() {
  ...
  // lcd.clear();
  lcd.setCursor(0, 2);
  String message1 = " REVOLUTIONIZE PLANT";
  for (int i = 0; i < message1.length(); i++) {
    lcd.print(message1[i]);
    delay(300);
  }
  lcd.setCursor(6, 3);
  String message2 = " CARE !!";
...
String predictCropSuitability(float temperature, float humidity, int moisture) {
  ...
  if ((temperature >= 15 && temperature <= 27) && (humidity >= 13 && humidity <= 14) && (moisture >= 21 && moisture <= 40)) {
    ...
    return "Potato";
  } else if ((temperature >= 24 && temperature <= 32) && (humidity >= 75 && humidity <= 90) && (moisture >= 65 && moisture <= 75)) {
    return "LadyFinger";
    ...
    return "Groundnut";
  } else if ((temperature >= 32 && temperature <= 37) && (humidity >= 63 && humidity <= 67) && (moisture >= 65 && moisture <= 71)) {
    return "Sweet Potato";
  } else if ((temperature >= 24 && temperature <= 32) && (humidity >= 70 && humidity <= 90) && (moisture >= 57 && moisture <= 65)) {
    return "Cabbage";
  } else {
    return "";
  }
...
void DisplayCycle(String PhaseOneReading, String PhaseTwoReading, String PhaseThreeReading, String PhaseFourReading, String totalReading) {
  ...
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Stem Extension Phase:");
  lcd.setCursor(0, 1);
...
void predictCropCultivationCycle(String crop) {
  ...
  if (crop == "Rice and Wheat" || crop == "Maize and Pulses" || crop == "Sweet Potato") {
